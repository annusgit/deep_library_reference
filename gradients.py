...
def _dot_gradient(op, grad):
    """Computes the gradients for `dot`.

    Args:
      op: The `dot` `Operation` that we are differentiating
      grad: Gradient with respect to the output of the `dot` op.

    Returns:
      Gradients with respect to the input of `dot`.
    """
